...
### 한 것
1. 게시판 댓글 기능 추가
2. Querydsl 설정을 2중 상속이 아닌 Config와 클래스 하나로 설정 -> 왜? -> 이중 상속보단 클래스 하나가 코드가 훨씬 깔끔하기 때문이다.

### Trouble Shooting
1. 코드 구현 과정에서 에러잡기에 많은 시간을 쏟은 것은 없지만, jpa의 지연로딩(FetchType.EAGER, FetchType.LAZY)와 영속성 전이(CascadeType.REMOVE 등)의 개념을 더 익혀야 할 필요성을 느꼈다. 지금까지 코드 중에서는 다대일 관계의 테이블이 없어서 많이 사용되지 않았었는데, 이젠 깊게 공부를 해야 할 것 같다. 처음에 쌩으로 김영한 님의 jpa 책을 읽었을 때는 영속성 전이 등 이해가 하나도 가지 않았는데, 이젠 구현해봤으니 이해가 조금은 쉬워질 것이라 느낀다. 
